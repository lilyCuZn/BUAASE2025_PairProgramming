...
let train_time1: i32 = 3;
let train_time2: i32 = 20;
...
function Manhattan(x1: i32, y1: i32, x2: i32, y2: i32): i32 {
    return <i32>(Math.abs(x1 - x2) + Math.abs(y1 - y2));
}
...
function run_shadow(deep: i32, n: i32, snake: i32[], other_snake_num: i32, now_other_num: i32, other_snake: i32[]): Pair {
    // 初始化 shadow_map
    if (mode == 1) {
...
export function greedy_snake_step(n: i32, snake: i32[], snake_num: i32, other_snake: i32[], food_num: i32, food: i32[]): i32 {
    ...
    for (let i = 0; i < snake_num; i++) {
        ...
        if (other_snake[2*i] + 1 < n) a[other_snake[2*i] + 1][other_snake[2*i+1]] = 2;
        if (other_snake[2*i] - 1 >= 0) a[other_snake[2*i] - 1][other_snake[2*i+1]] = 2;
        if (other_snake[2*i+1] + 1 < n) a[other_snake[2*i]][other_snake[2*i+1] + 1] = 2;
        if (other_snake[2*i+1] - 1 >= 0) a[other_snake[2*i]][other_snake[2*i+1] - 1] = 2;
    }
    if (mode == 1) {
        console.log("2");
    }
    ...
    for (let i = 0; i < food_num; i++) {
        ...
        let m: Move = A_star(n, snake, snake_num, other_snake, aX, aY); 
        if (minn > m.F && m.F > 0) {
            minn = m.F;
            dir = m.first;
        }
...
function A_star(n: i32, snake: i32[], snake_num : i32, other_snake:i32[] , aX: i32, aY: i32): Move {
    ...
    for (let i = 0; i <= n; i++) {
        if (mode == 1) {
            console.log("A_star: initial" + i.toString());
        }
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (mode == 1) {
            console.log("A_star circulation1: " + i.toString());
        }
...
next.heart = cal_heart(n,next.x,next.y);
         ...
         valueF[newX][newY] = next.F;
            ...
            next.H = length;
            if (mode == 1) {
                console.log("run_shadow begin!");
            }
            
            if (mode == 1) {
                console.log("run_shadow end!");
            }
            next.snake[0] = newX;
            next.snake[1] = newY;
    ...
    while (q.length > 0) {
        ...
        if (current.x == aX && current.y == aY) {
            if (mode == 1) {
                console.log("return current: " + current.first.toString() + " " + current.F.toString());
            }
            return current;
        }
        ...
        for (let i: i32 = 0; i < 4; i++) {
            ...
            if (newX > 0 && newX <= n && newY > 0 && newY <= n && a[newX][newY] == 0 && !visit[newX][newY]) {
                let next: Move = new Move(newX, newY);
                next.G = current.G + 1;
                next.H = Manhattan(newX, newY, aX, aY);
                next.first = current.first;
    //
                 next.S = run_shadow(train_step, n, snake, snake_num, snake_num, other_snake);
            //    next.F = next.G + next.H + cal(next.S);
//next.F = next.G + next.H;
next.heart = cal_heart(n,next.x,next.y);
next.F = next.G + next.H + next.heart;
                if (next.F < valueF[newX][newY] || valueF[newX][newY] == 0) {
                    valueF[newX][newY] = next.F;
                    next.snake[0] = newX;
                    next.snake[1] = newY;
                    ...
                    // 插入优先队列
                    q.push(next);
                    q.sort((a, b) => (a.F - b.F) ? 1 : -1); // 按F值从小到大排序
                }
            }
        }
